...
class TestPricingMerger:
    ...
    def test_merge_output_is_v7_format(self, sample_openai_data, tmp_path):
        ...
        usd = model["USD"]
        assert "text" in usd
        assert usd["text"]["in"] == pytest.approx(2.50)
        assert usd["text"]["out"] == pytest.approx(10.00)
    ...
    def test_merge_multiple_sources_priority(
        ...
        sample_openrouter_data["models"]["openai/gpt-4o"]["endpoints"]["openrouter.ai"]["pricing"]["in"] = 3.00
        ...
        assert result["models"]["gpt-4o"]["USD"]["text"]["in"] == pytest.approx(2.50)
    ...
    def test_merge_chinese_providers(
        ...
        assert result["models"]["qwen-max"]["CNY"]["text"]["in"] == pytest.approx(40.0)
    ...
    def test_small_drift_no_warning(self, sample_openai_data, tmp_path):
        ...
        close_data = {
            ...
            "models": {
                "gpt-4o": _model(
                    "api.openai.com",
                    {"in": 2.51, "out": 10.00},  # 0.4% difference
                    base_url="https://api.openai.com/v1",
                    metadata={"provider": "openai"},
                ),
            },
...
class TestNormalization:
    ...
    def test_unit_field_removed(self, tmp_path):
        ...
        model = _model(
            "api.openai.com",
            {"in": 2.50, "out": 10.00, "unit": "per_million_tokens"},
            metadata={"provider": "openai"},
        )
...
class TestManualOverrides:
    ...
    def test_manual_overrides_override_openrouter(self, tmp_path):
        ...
        assert result["models"]["gpt-4o"]["USD"]["text"]["in"] == pytest.approx(9.99)
...
class TestFieldLevelEnrichment:
        ...
        litellm_data = {
            ...
            "models": {
                "gpt-4o": _model(
                    ...
                    {"in": 2.50, "out": 10.0},
                    tiered_pricing=[
                        {"tier_start": 0, "tier_end": 128000, "in": 2.5, "out": 10.0},
                        {"tier_start": 128000, "in": 5.0, "out": 10.0},
                    ],
                    metadata={"provider": "openai"},
                ),
            },
    ...
    def test_winner_batch_pricing_not_overwritten(self, tmp_path):
        ...
        litellm_data = {
            ...
            "models": {
                "gpt-4o": _model(
                    "litellm",
                    {"in": 2.50, "out": 10.0},
                    batch_pricing={"in": 0.99, "out": 3.0},  # different
                    metadata={"provider": "openai"},
                ),
            },
        }
        ...
        usd = result["models"]["gpt-4o"]["USD"]
        assert usd["batch"]["in"] == pytest.approx(1.25)
...
class TestDerivedPricing:
    ...
    def test_batch_supported_model_gets_batch_pricing(self, tmp_path):
        ...
        assert cny["batch"]["out"] == pytest.approx(2.5)
...
class TestPricingCompletenessCheck:
    ...
    def test_completeness_warning_when_usd_has_batch_cny_does_not(self, tmp_path):
        ...
        model_usd = _model(
            ...
            {"in": 0.27, "out": 1.10},
            currency="USD",
            batch_pricing={"in": 0.135, "out": 0.55},
            metadata={"provider": "deepseek"},
        )
        model_cny = _model(
            "api.deepseek.com",
...
class TestAnomalyFilter:
    ...
    def test_image_uses_500_threshold(self, tmp_path):
        ...
        model_ok = _ep(
            "generativelanguage.googleapis.com",
            {"text": {"in": 1.0, "out": 2.0}, "image": {"out": 400.0}},
            currency="USD",
        )
    ...
    def test_image_above_threshold_rejected(self, tmp_path):
        ...
        model_bad = _ep(
            "generativelanguage.googleapis.com",
            {"text": {"in": 1.0, "out": 2.0}, "image": {"out": 600.0}},
            currency="USD",
        )
    ...
    def test_mixed_modality_anomaly_rejects_whole_model(self, tmp_path):
        ...
        model = _ep(
            "generativelanguage.googleapis.com",
            {"text": {"in": 1.0, "out": 2.0}, "image": {"out": 999.0}},
            currency="USD",
        )
